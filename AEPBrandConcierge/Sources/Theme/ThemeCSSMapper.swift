...
public enum CSSKeyMapper {
    ...
    /// The literal below covers everything except the 4 gradient-capable input tokens (border outline,
    /// mic icon, send-arrow background, mic waveform), whose 3-key (start/end/angle) shape is instead
    /// generated by `gradientAssignments` and merged in below -- see that function for why.
    private static let cssToAssignmentMap: [String: Assignment] = {
        ...
        "input-outline-color": { cssValue, theme in
            // Legacy themes may still send the old (never-actually-supported) "linear-gradient(...)"
            // form here; parsing that as a hex color would silently produce the wrong solid color
            // (Color.fromHexString's fallback). Ignore it -- migrate to the *-gradient-* keys instead.
            guard !cssValue.hasPrefix("linear-gradient") else {
        ...
        let gradientTokens: [(cssKeyPrefix: String, keyPath: WritableKeyPath<ConciergeInputColors, ConciergeGradient?>)] = [
            ("input-outline-gradient", \.outlineGradient),
            ("input-send-arrow-background-gradient", \.sendArrowBackgroundGradient),
            ("input-mic-icon-gradient", \.micIconGradient),
            ("input-mic-waveform-gradient", \.micWaveformGradient),
        ]
        for token in gradientTokens {
            map.merge(gradientAssignments(cssKeyPrefix: token.cssKeyPrefix, keyPath: token.keyPath)) { _, new in new }
        }
        return map
    }()
    ...
    /// Applies one field-mutation on top of whatever gradient (if any) prior CSS keys already built,
    /// defaulting a not-yet-configured gradient to clear/clear (see `ConciergeGradient.isRenderable`,
    /// which treats that placeholder as "not yet active" until both colors are real).
    private static func withGradient(_ existing: ConciergeGradient?, _ mutate: (inout ConciergeGradient) -> Void) -> ConciergeGradient {
        var gradient = existing ?? ConciergeGradient(startColor: CodableColor(.clear), endColor: CodableColor(.clear))
        mutate(&gradient)
        return gradient
    }

    /// Generates the 3 CSS key assignments (start color, end color, angle) that together configure one
    /// `ConciergeGradient?` property, keyed by a shared CSS variable prefix (ex: "input-outline-gradient"
    /// -> "input-outline-gradient-start-color", "-end-color", "-angle"). Every gradient-capable input
    /// token (border outline, mic icon, send-arrow background, mic waveform) shares this exact shape.
    private static func gradientAssignments(
        cssKeyPrefix: String,
        keyPath: WritableKeyPath<ConciergeInputColors, ConciergeGradient?>
    ) -> [String: Assignment] {
        [
            "\(cssKeyPrefix)-start-color": { cssValue, theme in
                theme.colors.input[keyPath: keyPath] = withGradient(theme.colors.input[keyPath: keyPath]) {
                    $0.startColor = CSSValueConverter.parseColor(cssValue)
                }
            },
            "\(cssKeyPrefix)-end-color": { cssValue, theme in
                theme.colors.input[keyPath: keyPath] = withGradient(theme.colors.input[keyPath: keyPath]) {
                    $0.endColor = CSSValueConverter.parseColor(cssValue)
                }
            },
            "\(cssKeyPrefix)-angle": { cssValue, theme in
                theme.colors.input[keyPath: keyPath] = withGradient(theme.colors.input[keyPath: keyPath]) {
                    $0.angle = CSSValueConverter.parseGradientAngle(cssValue)
                }
            },
        ]
    }
